...
def df_preview_debug(df):
    ...
    track_one_value = True
    #track_one_value = False
...
csv_file1="mp3_encoder.csv"
csv_file2="mp3_offset.csv"
...
csv_df = df_diff_columns(csv_df, 'fhg_default', 'mpg_default',   'diff_default')
csv_df = df_diff_columns(csv_df, 'fhg_default', 'mpg_nogapless', 'diff_nogapless')

csv_columns=['stem'] + encoders_list + ['diff_default', 'diff_nogapless']
info_df = csv_df[csv_columns]

info_df = info_df.query('abs(diff_default) < 1000 ')  # remove impossible shifts
df_preview_debug(info_df)

#csv_columns=['stem', "encoder", 'diff_nogapless']
#info_df = csv_df[csv_columns]
#df_preview_debug(info_df)



plot_df = pd.merge(info_df, merge_df, on='stem')
plot_df['predicted_offset'] = plot_df['diff_nogapless'] + plot_df['RB_vs_TK_adj']    # FIXME: polarity got wrong somewhere
df_preview_debug(plot_df)
...
def do_cdf_plot(df, *, y=None, hue=None, do_abs=False, lim=500, **kwargs):
    ...
    seaborn_cdfplot(df, y=y, hue=hue, height=4, aspect=3, ylim=ylim, **kwargs)

display_versions(versions)    
    
for hue in [None]: #, "tag"]:
    do_cdf_plot(plot_df, y="diff_nogapless", hue=hue, title="'Fraunhofer' vs 'mp123 --nogap' Decoders")
    do_cdf_plot(plot_df, y="RB_vs_TK_adj", hue=hue, title="Traktor vs Rekordbox MP3 auto-beatgrid difference (adjusted for WAV)")
    do_cdf_plot(plot_df, y="predicted_offset", hue=hue, title="Predicted Offsets with Fraunhofer correction")
