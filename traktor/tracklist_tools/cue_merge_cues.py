...
def generate_cue(file_music, input_tl, input_cue):
  ...
  try:
    cd_name = file_music.split("-",1)[1].strip()
  except:
    cd_name = "untitled"
  ...
  if len(indexes) != count:
    if not opts.debug_mismatch_sizes:
      die("Cue has different size than Tracklist. Use -z to accept it by force")
    else:
...
def process_one_set(opts):
  ...
  if opts.tracklist:
    ...
    print("Reading tracklist from __CUE file itself__")
...
parser.add_argument('-T', dest="reload_standard_tracklist", default=False, action='store_true',
                    help='Read tracklist from standard basename (to avoid specifiyng it by hand)')
...
parser.add_argument('--offset', dest="cue_offset_minutes", default=0, type=int,
                    help='Apply an offset in minutes to the cue')
                    
parser.add_argument('--recursive', dest="recursive_operation", default=False, action='store_true',
